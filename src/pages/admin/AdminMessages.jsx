...
export default function AdminMessages() {
  ...
  const loadConversations = useCallback(async () => {
    const all = await messageService.listConversations();
  ...
  useEffect(() => {
    ...
    messageService
      ...
      .then((loadedMessages) => {
        ...
        const safeMessages = Array.isArray(loadedMessages)
          ? loadedMessages
          : [];
  ...
  useEffect(() => {
    const unsubscribe = wsClient.on(
      ...
      (payload) => {
        ...
        /*
         * Ignore an event that we already have.
         */
        if (
          messageId != null &&
          messageIds.current.has(String(messageId))
        ) {
          return;
        }
        ...
        if (conversationId === activeId) {
          ...
          if (messageId != null) {
            messageIds.current.add(String(messageId));
          }

          setMessages((previous) => {
            /*
             * Extra protection against duplicate websocket
             * events without an id.
             */
        ...
        /*
         * Refresh the sidebar so lastMessage / unread state
         * remains current.
         */
        messageService
          .listConversations()
  ...
  return (
    <div>
      <PageHeader
        title="Messages"
        subtitle="Respond to employee support requests."
      />
      ...
      {!conversations ? (
        ...
        <div
          className="flex h-[600px] overflow-hidden rounded-[var(--radius-lg)] border"
          style={{
